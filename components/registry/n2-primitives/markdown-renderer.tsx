...
function renderInline(text: string): string {
  ...
  result = result.replace(
    /`([^`]+)`/g,
    '<code class="rounded-[var(--radius-md,12px)] bg-muted px-1.5 py-0.5 font-mono text-sm">$1</code>'
  )
  // Bold + italic
  ...
  result = result.replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    '<a href="$2" class="text-cobalt underline underline-offset-4 transition-colors hover:text-cobalt/80" target="_blank" rel="noopener noreferrer">$1</a>'
  )
  return result
}
...
function parseMarkdown(content: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("```")) {
      if (inCodeBlock) {
        html.push(
          '<pre class="overflow-x-auto rounded-[var(--radius-xl,17px)] bg-muted/50 p-4 font-mono text-sm"><code>' +
